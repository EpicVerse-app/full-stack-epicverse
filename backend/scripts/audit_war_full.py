...
async def check():
    ...
    async with db_pool.acquire() as conn:
        rows = await conn.fetch('SELECT attribute_card_no, final_status, revised_scholar_reason FROM card_combos WHERE gameplay_mode = $1 AND character_card_number = $2 ORDER BY attribute_card_no', 
                               'WarRoom (YuddhaKanda)', 3)
        print(f"Full Audit for WarRoom - Lakshmana (3):")
        for r in rows:
