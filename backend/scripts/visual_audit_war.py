...
async def main():
    # DB Mapping for WarRoom: WarRoom (YuddhaKanda)
    mode = "WarRoom (YuddhaKanda)"
    
    # Test 1: Valid (Lakshmana + Exposure)
    await audit_combo(mode, "3", "42")
    
    # Test 2: Valid (Lakshmana + Acknowledgement)
    await audit_combo(mode, "3", "43")
    
    # Test 3: Valid but Excluded (Lakshmana + Verification)
    await audit_combo(mode, "3", "44")
