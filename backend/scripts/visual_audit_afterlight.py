...
async def audit_combo(mode, card1, card2, follow_up="Why?"):
    ...
    async with websockets.connect(uri) as websocket:
        ...
        await websocket.send(json.dumps({
            "type": "text_query",
            "text": f"(System: Speak ONLY in English) {card1} and {card2}"
        }))
...
async def main():
    # DB Mapping for AfterLight: AfterLight (UttaraKanda)
    mode = "AfterLight (UttaraKanda)"
    
    # Test 1: Valid (Bharata + Support)
    await audit_combo(mode, "5", "83")
    
    # Test 2: Valid (Bharata + Anger)
    await audit_combo(mode, "5", "87")
    
    # Test 3: Valid but Excluded (Hanuman + Duty)
    await audit_combo(mode, "4", "25")
